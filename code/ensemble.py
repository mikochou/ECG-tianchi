...
parser.add_argument('--config', default='./ensemble_dev_testB.yaml')
...
def main():
    ...
    # Test
        ...
        test(model_list, model_weight, args, mname, device)
    return
...
def test(model_list, model_weight, opt, mname, device):
    '''
    ...
    with open(sub_file, 'w', encoding='utf-8') as fout:
        with torch.no_grad():
            ...
            for line in tqdm(lines):
                ...
                output_list = []
                for index, model in enumerate(model_list):
                    if mname[index] in ['ResNeXt50_2x64d_Basic', 'ResNet50_Basic', 'ResNet101_Basic']:
                        outputs = model(data)
                    else:
